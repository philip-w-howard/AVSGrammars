...
We could continue to find new meta-states by enumerating all possible outbound inputs from each meta-state and then following the $\Lambda$s from the resulting states. The results are presented in Table~\ref{T.subsetTable1}. The first column gives the name of the meta-state. The second column gives the list of NFA states that make up the meta-state. The remaining columns, one per possible input, indicates what NFA states can result by following one instance of the input character (then $\Lambda$s).
...
\begin{enumerate}
\item Create the empty table with columns as shown in Table~\ref{T.subsetTable1}.
...
\item For each NFA state in the meta-state, if there is an outbound transition on the input, write down the destination state in the column for that input.
\item Extend the list of states by forming the $\Lambda$ closure of each state already in the cell.
\item If there are any new unique sets of states in the resulting states column, give them a unique label and return to Step~3.
\end{enumerate}
...
Completing the first row, we can follow an $a$ from state 1 to 5, and from state 2 to 3. Add these two states to the $a$s column, and then follow the $\Lambda$s adding state 6. Label the set \code{ \{3, 5, 6\}} as meta-state~$B$. 
Completing row 1, there are no $b$ nor $c$ transitions out of any of the states in meta-state~$A$, so the resulting states for both of these columns are empty.
...
Moving on to row 2, we can follow an $a$ from state 6 to state 4. We can then follow $\Lambda$s from 4 to 1 and 1 to 2 yielding meta-state $C$ containing states 1, 2, 4.

Wwe can follow a $b$ from 3 to 4 and from 5 to 2. We can then follow a $\Lambda$s from 4 to 1 and from there to 2 (which we've already reached), so this meta-state contains 1, 2 and 4, which we've already labeled $C$.

We can follow a $c$ from 5 to 6. There are no $\Lambda$s from 6, so meta-state~$D$ contains only 6. Adding these to the table, we have:
...
Completing row 3, we can follow an $a$ from 1 to 5 and 2 to 3. The $\Lambda$ closure adds 6. This set of states is already labeled $B$, so we don't need to add any rows to the table.
...
\textbf{Important note:} The procedure is to follow the input out of the NFA states and then follow the $\Lambda$s. A common mistake is to follow the $\Lambda$s out of the NFA states. For example, when filling out the B row, even though there is a $\Lambda$ from state 3, we don't follow it because we are only looking for transitions on $a$, $b$, or $c$.
...
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{From} & \textbf{a} & \textbf{b} & \textbf{c}\\
\hline
A & B & - & -\\
B & C & C & D\\
C & B & - & E\\
...
\end{tabular}
\end{center}
\caption[Transition table for DFA]{A summary of the table derived in this section. Each row contains the start state, and for each input character, the resulting state if that character is found while in the start state.}
...
\caption[Symbols for PDAs]{Symbols for PDAs.}
\label{F.pda.symbols}
\end{figure}
...
\pdaconnect  {1.5,4} {read2a}
\pdatransition {read2a}{read2}{}
...
\fanonterminalnode {5,2}{D};
\fanonterminalnode {5,0}{E+};
...
\begin{center}
\begin{tikzpicture}
\filldraw[color=red, fill=red, fill opacity=0.5, very thick] (-1,0) circle (1.5);
\filldraw[color=blue, fill=blue, fill opacity=0.5, very thick] (1,0) circle (1.5);
\draw (-1,0) node {a b};
\draw (0,0) node {c};
\draw (1,0) node {d e};
\end{tikzpicture}
