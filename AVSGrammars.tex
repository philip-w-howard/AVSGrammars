...
In this section we show that all grammars (for languages that don't include the empty string) can be transformed into Chompsky's Normal Form. In this form, all productions either have exactly two non-terminals on the right hand side (and no terminals) or exactly one terminal.
...
Now that we have a procedure for identifying all the nullables, we can give a procedure for eliminating $Lambda$ productions.
